  ...
  <div>
    ...
    <section class="viewRolling">
        <div  class="swipe-container">
            <van-swipe :loop="false" :width="swipeItemWidth" :height="swipeItemHeight" 
                 ref="VanSwipe">
            <template v-for="(val,index) in stream_list">
                <van-swipe-item class="swipe-item-container"  v-if="!val.seat"  :key="index">
                <!-- <van-swipe-item class="swipe-item-container"  v-show="!val.seat"  :key="index"> -->
                        <!-- 显示视频内容 -->
                        ...
                        <!-- 显示占位图片 -->
                        <section v-show="!val.setVideo" style="position: absolute;
                                top: 50%;
                                left: 50%;
                                transform: translate(-50%, -65%);}">
                            <van-image style="width:56px;height:56px;" 
                                :src="val.roleType == 'teacher'?teaPlaceHoldImgUrl:stuPlaceHoldImgUrl"></van-image>
                        </section>
                        <!-- 底部文字内容 -->
                        <div class="text-container">
                            ...
                            <p class="icon-container"  v-show="audioAndVideoIconFunc(val)">
                                <!-- 音频 -->
                                <van-image class="iconPoniter" 
                                    :style="speakVoiceUrlStatus?speakVoiceUrlStyle:normalSpeakVoiceUrlStyle" 
                                    :src="audioSpeakIconFunc(val)" @click="swipeAudioFunc(val,index)"></van-image>

                                <!-- 视频 -->
                                <van-image class="iconPoniter" style="width:12px;height:14px;" 
    ...
    <main>
       ...
       <!-- 右侧浮层 -->
                ...
                <com-tool @handleClick="handleToolsClick" v-if=" userType == 'students' ? false : true " 
                    style="position:absolute;bottom:45px;right:16px;" />
...
export default {
    ...
    data(){
        return {
            ...
            teaPlaceHoldImgUrl:require("@/assets/u45.png"),//老师默认占位图片
            stuPlaceHoldImgUrl:require("@/assets/u46.png"),//学生默认占位图片
            stream_list:[
                /**
                    * 
            ...
            speakVoiceUrlStatus:false,
            normalSpeakVoiceUrlStyle:{
                width:'10.5px',
                height:'14.5px',
                marginRight:'10px'
            },
    ...
    computed:{
        ...
        audioSpeakIconFunc(){
            return function(item){
                if(item.isSpeak){
                    this.speakVoiceUrlStatus = true;
                    return this.speakVoiceUrl;
                }else{
                    this.speakVoiceUrlStatus = false;
                    return item.setAudio ? this.TvoiceUrl : this.closeVoiceUrl;
                }
            }
        },
        listData() {
            return this.$store.state.desktopStatus;
        }
    },
    watch:{
    ...
    methods:{
        ...
        init_emedia_callback(){
            let that = this;
            const {userName,accessToken,confrId} = this.$store.state.user;
            ...
            emedia.mgr.onStreamAdded =  async function (member, stream) {
            // emedia.mgr.onStreamAdded =  function (member, stream) {
                ...
                if(!stream.located()) {
                    if( member.memName.split('.').length > 1){//进来的是老师
                            ...
                            // 获取用户昵称
                            const result = await GetNickName(confrId,stream.owner.memName);
                            /**
                             * 此种方法绑定老师名称 当屏幕共享时会报错，由于没有stream.ext.nickName
                             * 采用如上接口调用方式绑定老师名称
                             */ 
                            if(stream.type == 0){// 0 未开启屏幕共享 1 已开启屏幕共享
                                ...
                                setTimeout(() => {
                                    let result;
                                    let arrayData = that.stream_list;
                                    let data= arrayData.filter(function(item){
                                        return item.roleType == "teacher"; 
                                    });
                                    let datas= arrayData.filter(function(item){
                                        return item.roleType !== "teacher" && item.roleType !== "teacher-seat"; 
                                    });
                                    data.map( res => {
                                        result = datas.unshift(res);
                                    });
                                    that.stream_list = datas;
                                    emedia.mgr.subscribe(member, stream, true, true, document.getElementById("list-"+ member.name));
                                }, 10); 
                                setTimeout(() => {
                                    emedia.mgr.subscribe(member, stream, true, true, document.getElementById("list-"+ member.name));
            ...
            emedia.mgr.onStreamRemoved = function(member, stream) {
                ...
                console.log('媒体流发布者退出');
                console.log(member);
                console.log(stream);
                console.log("媒体流发布者退出=========" + JSON.stringify(member.name));
                let teaMember = member.name.split(".");
                if(teaMember.length > 1){
                    // 老师退出的流 
                    document.getElementById("desktop").style.zIndex=0;
                }
            ...
            emedia.mgr.onConferenceExit = function (reason, failed) {
                ...
                if(this.shared_desktop) {//如果状态为共享屏幕
                    this.stop_share_desktop();
                }
        ...
        async swipeAudioFunc(scope,index){
            ...
            if(scope.setAudio){
                if(userType == "teacher"){ //如果角色为老师 老师可以操作学生音频流 
                    //解除静音某一人
                    emedia.mgr.unmuteBymemberId(confrId, scope.member.id);//老师开启别人音频
                    const res_stream = await emedia.mgr.resumeAudio(scope.stream);//老师开启自己音频
                }else{
                    const res_stream = await emedia.mgr.resumeAudio(scope.stream);
                }
            }else{
                if(userType == "teacher"){ //如果角色为老师 老师可以操作学生音频流 
                   // 静音某一人
                    emedia.mgr.muteBymemberId(confrId, scope.member.id);//老师关闭别人音频
                    const res_stream = await emedia.mgr.pauseAudio(scope.stream) //老师关闭自己音频
                }else{
                    const res_stream = await emedia.mgr.pauseAudio(scope.stream) //关闭音频
                }
        ...
        async swipeVideoFunc(scope,index){
            ...
            if(scope.setVideo){
                if(userType == "teacher"){
                     ...
                     * 环信SDK不支持管理员操作别人视频流
                    */
                   // 由于环信当前sdk不支持管理员操作被人的视频流，所以现在老师只可以控制自己的视频流，控制学生的视频流无效
                    if(scope.roleType == "teacher"){
                      const res_stream = await emedia.mgr.resumeVideo(scope.stream) //老师开启自己的视频
                    }
                }else{
                    const res_stream = await emedia.mgr.resumeVideo(scope.stream) //学生开启视频
                }
            }else{
                if(userType == "teacher"){ 
                    /**
                    ...
                    // 由于环信当前sdk不支持管理员操作被人的视频流，所以现在老师只可以控制自己的视频流，控制学生的视频流无效
                    if(scope.roleType == "teacher"){
                       const res_stream = await emedia.mgr.pauseVideo(scope.stream) //老师关闭自己的视频
                    }
                }else{
                   const res_stream = await emedia.mgr.pauseVideo(scope.stream) //学生关闭视频
                }
            }
        },
        ...
        async setWhiteBoradFunc (scope,index){
            ...
            stream_list.map(item=>{
                if(item.roleType == 'teacher'){
                    let teacherName = item.stream.owner.name;
                    let getTeacherName = teacherName.split("_");
                    if(getTeacherName.length > 1){
                        this.saveStreamListName = getTeacherName[1];
                    }else{
                        this.saveStreamListName = getTeacherName[0];
                    }
                }
            });
        ...
        async setMessageFunc (scope,index){
            ...
            if(scope.setMessage){
                const result = await this.setChatroomMute(obj);
                console.log(result,'==>result')
                console.log(scope,'==>scope')
                let chatMember = JSON.parse(window.localStorage.getItem("chatMember"));
                console.log(chatMember,'==>chatMember')

                scope.setMessage = false;
                
            }else{
                const result = await this.removeRoomMute(obj);
                console.log(result,'==>result')
        ...
        //关闭学生列表窗口
        async closeStuListFunc(){
            this.stuListStatusOverlay = false;
            let result = await this.getRoomMuteList();
            if(result.data.length !== 0){  // 有人被禁言了
