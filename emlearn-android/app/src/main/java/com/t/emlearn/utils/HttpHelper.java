    ...
    public static String post(String url, String content,String contentType) {
        ...
        try {
            ...
            if(conferenceSession != null){
                String sysToken = conferenceSession.getSysToken();
                if(sysToken != null && !sysToken.isEmpty()){
                    conn.setRequestProperty("Authorization", "Bearer "+sysToken);
            ...
            if (responseCode == 200) {
                ...
                InputStream is = conn.getInputStream();
                String response = getStringFromInputStream(is);
                throw new NetworkErrorException("response status is "+responseCode+" resp "+response);
            }
